...
func run(ctx context.Context, args []string, stdout, stderr io.Writer) int {
	...
	flags.StringVar(&opts.Skill, "skill", "", "repository skill name, or local skill directory when --local is set")
	...
	flags.BoolVar(&opts.ListSkills, "list-sk", false, "list skills available under the repository skills/ directory")
	flags.BoolVar(&opts.Local, "local", false, "install the local directory provided by --skill instead of downloading it")
	...
	flags.Usage = func() {
		...
		fmt.Fprintln(stderr, "  rayskills --model codex --local --skill ./my-skill")
		fmt.Fprintln(stderr, "  rayskills --list-sk")
