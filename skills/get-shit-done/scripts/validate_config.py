#!/usr/bin/env python3
"""
validate_config.py — Validate an .config.yaml file against company rules.

Usage:
    python validate_config.py <path/to/.config.yaml> [--repo-root <dir>] [--json]
...
The real config schema is internal, so the rules here are a reasonable
...
# ─────────────────────────────────────────────────────────────────────────────
# RULES — EDIT THIS SECTION to match your real .config.yaml schema.
# ─────────────────────────────────────────────────────────────────────────────
#
...
def validate(path: str, repo_root: str):
    ...
    if not isinstance(data, dict):
        print("ERROR: top level of .config.yaml must be a mapping.", file=sys.stderr)
        sys.exit(2)
...
def main():
    ap = argparse.ArgumentParser(description="Validate an .config.yaml file.")
    ap.add_argument("file", help="Path to .config.yaml")
    ...
    if args.json:
        ...
        if not findings:
            print("✅ .config.yaml is valid. No issues found.")
        else:
            for f in errors:
